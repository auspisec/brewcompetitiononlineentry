...
/**
 * Seoul Cup: entry labels must be uniform regardless of the requesting
 * user's UI language choice, so labels always render in the site-wide
 * default language (privacy: no identifying the entrant's language from
 * the label). Swap the language session vars to the default for the
 * duration of this output render, then restore them afterwards.
 * The userLanguage cookie is neutralized too, because
 * language.lang.php re-applies it below our swap point.
